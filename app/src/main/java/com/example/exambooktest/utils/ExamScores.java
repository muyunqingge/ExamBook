...
public class ExamScores {
    ...
    public ExamScores(String studentId, ExamScores.CallBack callBack) {
        new Thread(() -> {
            MySQL mySQL = new MySQL();
            //获得所有数据
            String sql = "select score, date from exam where student_id = " + studentId;
            //获得最高分
            String sql2 = "select max(score) from exam where student_id = " + studentId;
            //获得及格数据
            String sql3 = "select score, date from exam where score >= 60 and student_id = " + studentId ;
            //获得当前数据
            String sql4 = "select score from exam where student_id = " + studentId + " order by id desc";
            try {
